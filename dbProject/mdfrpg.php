...
<body>
	<nav id="fixheight" class="navbar navbar-expand-lg navbar-light bg-light">
  		...
  		<form id="fixsearch" class="form-inline" action="func/rpgSearch.php" method="POST">
    		<input class="form-control mr-sm-2" type="search" name="rpgSearch" placeholder="Search RPG" aria-label="Search">
    		<button class="btn btn-outline-success my-2 my-sm-0" type="submit" name="rpgSubmit" >Search</button>
  		</form>
	</nav>
  ...
  <form id="mdfrpgForm" action="func/mdfRpg.php" method="POST">
	<?php
    ...
    if(isset($_SESSION['mdfrpg']) and !empty($_SESSION['mdfrpg'])){
        echo "<script>alert('".$_SESSION['mdfrpg']."');</script>";
        $_SESSION['mdfrpg']="";
    }
        ...
        for(k=0;k<a.length;k++){
        	(function(k){
           		a[k].addEventListener("click",function(){
              		...
              		document.querySelector("#mdfrpgForm").submit();
           		});
        	})(k);
       }
