...
<body>
	<nav id="fixheight" class="navbar navbar-expand-lg navbar-light bg-light">
  		...
  		<form id="fixsearch" class="form-inline" action="func/rpgSearch.php" method="POST">
    		<input class="form-control mr-sm-2" type="search" name="rpgSearch" placeholder="Search Game" aria-label="Search">
    		<button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
  		</form>
	</nav>
  <form id="delrpgForm" action="func/delrpg.php" method="POST">
	<?php
  ...
  <?php
   if(!isset($_SESSION['showRPGSearch']) or $_SESSION['showRPGSearch'] === False){
      $query = ('select * from rpg');
   }
   else if($_SESSION['showRPGSearch'] === True){
      $query = ('select * from rpg where Name like "%'.$_SESSION['rpgSearch'].'%"');
  ...
  for($i=0; $i<count($result); $i++){
    ...
    echo"<th scope='row'><input class=\"gameRadio\" type=\"radio\" name=\"mdfrpg\" value=\"\" checked=\"\"><a class=\"radiohide\" style=\"margin-right:5px;\" href=\"#\"><span style=\"font-size: 4px;color:rgb(167,171,175);display:inline-block;\"><i class=\"far fa-trash-alt\"></i></span></a></th>";
  ...
  echo "</table>"
  ?>
  <input id="delwhichrpg" type="hidden" name="delrpg" value="">
  </form>
  ...
  <script type="text/javascript">
    ...
    var delcardform = document.querySelector("#delrpgForm");
    var delcard = document.querySelector("#delwhichrpg");
    lastclick = -1;
