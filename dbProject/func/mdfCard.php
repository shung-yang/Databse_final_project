<?php
	...
	if($oldname != $_POST["mdfname"]){
		$query = ('select * from game where Name = ? and Type <> "CARD"');
        $stmt = $db->prepare($query);
        $stmt->execute(array($_POST["mdfname"]));
        $result = $stmt->fetchAll();
        if(count($result) != 0){
            $_SESSION['mdfcard'] = "該遊戲已存在於其他類型的表格中";
            $cantchg = true;
        }
	}
	if($oldname != $_POST["mdfname"] || $oldwebsite != $_POST["mdfwebsite"]){
		$query = ('select * from card where Name = ? and Website = ?;');
    	$stmt = $db->prepare($query);
    	$stmt->execute(array($_POST["mdfname"],$_POST["mdfwebsite"]));
    	$result = $stmt->fetchAll();
    	if(count($result) != 0){
      	 $_SESSION['mdfcard'] = "該遊戲資源已存在";
      	 $cantchg = true;
    	}
	}
    ...
    if($oldname != $_POST["mdfname"]){
        ...
        $result = $stmt->fetchAll();
        if(count($result) != 0){
        	$query = ('update card set Name = ? where Name = ? and Website = ?;');
		    $stmt = $db->prepare($query);
